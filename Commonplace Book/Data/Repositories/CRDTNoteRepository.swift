...
actor CRDTNoteRepository: NoteRepository, CRDTNoteRepositoryProtocol {
    ...
    private nonisolated(unsafe) let idColumn = SQLite.Expression<String>("id")
    private nonisolated(unsafe) let crdtDataColumn = SQLite.Expression<Data>("crdt_data")
    private nonisolated(unsafe) let filePathColumn = SQLite.Expression<String>("file_path")
    private nonisolated(unsafe) let lastModifiedColumn = SQLite.Expression<Int64>("last_modified")
    // Columns for efficient searching without loading CRDTs
    private nonisolated(unsafe) let titleColumn = SQLite.Expression<String>("title")
    private nonisolated(unsafe) let contentColumn = SQLite.Expression<String>("content")

    // Tombstone table for delete tracking
    private nonisolated(unsafe) let tombstonesTable = Table("tombstones")
    private nonisolated(unsafe) let tombstoneIdColumn = SQLite.Expression<String>("id")
    private nonisolated(unsafe) let tombstoneDeletedAtColumn = SQLite.Expression<Int64>("deleted_at")
